...
describe('Testing profile picture upload', () => {
	it('Should get a status of 200', (done) => {
		chai.request('http://localhost:4000').post('/profile/uploadmulter').attach( 'image', 'C:\Users\QAKrisDuff\Documents\Github\Pull Requests\QA-Concourse-Backend\Pizza.jpg').end((err, res) => {
			if (err) {
                console.log(err);
